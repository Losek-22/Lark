...
class LogInWindow:
    ...
    def registerwindow(self, event):
        ...
        self.passwordEntry.delete(0, tk.END)
        self.passwordEntry.bind('<Return>', self.registerattempt)
        self.loginEntry.delete(0, tk.END)
        self.loginEntry.focus()
...
class MainChatWindow:
    ...
    def __init__(self, main):
        ...
        # allows for continous message flow alongside tkinter mainloop
        self.newmessage = Thread(target=self.receivemessage)
        self.newmessage.daemon = True
        self.newmessage.start()

    def receivemessage(self):
        """Receives a global message, puts it into the main message box."""

        while True:
            self.message = s.recv(4096).decode('utf8')
            user = self.message.split('-')[0]
            text = self.message.split('-')[1].strip()
            self.chatbox.configure(state='normal')
            self.chatbox.insert(tk.END, f'{user}: ', 'user')
    ...
    def sendmessage(self, event):
        """Sends a message destinated to all online users."""
    ...
    def lighttheme(self):
        """Changes chatbox window theme to light."""
